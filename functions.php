...
define('THEMEVER', "2.1.1"); 

//引入文件
require_once('aq_resizer.php');
require_once(TEMPLATEPATH . '/functions/bymt-update.php');
$bymt_update_checker = new ThemeUpdateChecker(
	'BYMT',
	'http://cdn.yuxiaoxi.com/theme/update.json'
);
...
function bymt_enqueue_scripts() {
	if( !is_admin() ) {
		if(wp_is_mobile()){
			wp_enqueue_script('mobile', TPLDIR . '/js/mobile.min.js', array(), THEMEVER, false);
			if (is_singular()){
			...
			wp_enqueue_script( 'global', TPLDIR . '/js/global.min.js', array(), THEMEVER, true);
			if(bymt_c('lazyload')){
				wp_enqueue_script('lazyload', TPLDIR . '/js/lazyload.min.js', array(), THEMEVER, true);
			}
			if (is_singular()){
				wp_enqueue_script('comments', TPLDIR . '/js/comments.min.js', array(), THEMEVER, true);
			}
			if(bymt_c('ajax_ggdc')&&bymt_c('ggid')){
				wp_enqueue_script('commvote', TPLDIR . '/js/commvote.min.js', array(), THEMEVER, true);
			}
			if(bymt_c('lightbox_ck') && is_singular()){
				wp_enqueue_script('lightbox', TPLDIR . '/js/lightbox.min.js', array(), THEMEVER, true);
			}
			if(bymt_c('wpshare')){
				wp_enqueue_script('wpshare', TPLDIR . '/js/wpshare.min.js', array(), THEMEVER, true);
			}

		}
		if(bymt_c('highlight')){
			if (is_singular()){
				wp_enqueue_style('highlight', TPLDIR . '/css/highlight.min.css', array(), THEMEVER, 'all');
		...
		if (is_category('picturewall')){
			wp_enqueue_script( 'wookmark', TPLDIR . '/js/wookmark.min.js', array(), THEMEVER, true);
			wp_enqueue_script( 'imagesloaded', TPLDIR . '/js/imagesloaded.min.js', array(), THEMEVER, true);
			wp_enqueue_script( 'picturewall', TPLDIR . '/js/picturewall.min.js', array(), THEMEVER, true);
		}
	}
}
...
function custom_login() {
	echo '
<link rel="stylesheet" type="text/css" href="'. TPLDIR .'/css/login.min.css" />';
}
