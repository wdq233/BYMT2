jQuery(document).ready(function($) {
	...
	if (0 < $("#comment_form").length) {
		...
		u = '<div id="loading"><img src="' + (k + "/wp-content/themes/BYMT/images/loading.gif") + '" alt=""/> \u6b63\u5728\u63d0\u4ea4, \u8bf7\u7a0d\u5019...</div>',
		...
		$("#loading").hide();
		$("#error").hide();
		$(".commentlist").length || $(".respond").before('<ol class="commentlist"></ol>');
		s.submit(function() {
			$("#comment").attr("readonly", !0);
			$("#loading").slideDown();
			f.attr("disabled", !0).fadeTo("slow", 0.65);
			i && $("#comment").after('<input type="text" name="edit_id" id="edit_id" value="' + i + '" style="display:none;" />');
			$.ajax({
				url: k,
				...
				error: function(b) {
					$("#loading").slideUp();
					$("#error").slideDown().html(t + b.responseText);
					setTimeout(function() {$("#comment").attr("readonly", !1);f.attr("disabled", !1).fadeTo("slow", 1);$("#error").slideUp()},3E3);
				},
				success: function(b) {
					$("#loading").hide();
					...
					ok_htm = '\n<span id="success' + v;
					ok_htm += "</span>\n";
					"0" == o ? $(".commentlist").append(new_htm) : $("#respond").before(new_htm);
					$("#new_comm_" + d).hide().append(b);
	...
	if ($("#wrapper-inner").attr("data-bymts").indexOf("ajax-comment") >= 0) {
		...
		l.on("click", ".pagination a",function(b) {
			b.preventDefault();
