...
function bymt_comment_length( $comment ) {
	...
	if ( $point_length < $min_length )
        {
			err( __('抱歉,您的评论太短了,请至少输入' . $min_length .'个字(已输入'. $point_length .'个字)') );
        }
	if ( $point_length > $max_length )
        {
			err( __('抱歉,您的评论太长了,请不要超过' . $max_length .'个字(已输入'. $point_length .'个字)') );
        }
	return $comment;
}
...
function bymt_comment_post( $comment ) {
	...
	if(bymt_c('lang_en')){ 
		if(!preg_match($pattern_en, $content)) {
			err(__('请写点汉字吧！ Please write some Chinese！'));
		}
	}
	if(bymt_c('lang_ja')){ 
		if(preg_match($pattern_ja, $content)) {
			err(__('日文滚粗！Japanese Get out！日本語出て行け！'));
		}
	}
	if(bymt_c('lang_ru')){ 
		if(preg_match($pattern_ru, $content)) {
			err(__('战斗民族伤不起！Russians, get away！Savage выйти из Русского Севера!'));
		}
	}
	if(bymt_c('lang_kr')){ 
		if(preg_match($pattern_kr, $content)) {
			err(__('不要用韩语思密达！Please do not use Korean！하시기 바랍니다 한국 / 한국어 사용하지 마십시오！'));
		}
	}
	if(bymt_c('lang_th')){ 
		if(preg_match($pattern_th, $content)) {
			err(__('人妖你好，人妖再见！Please do not use Thai！กรุณาอย่าใช้ภาษาไทย！'));
		}
	}
	if(bymt_c('lang_ar')){ 
		if(preg_match($pattern_ar, $content)) {
			err(__('不要用阿拉伯语！Please do not use Arabic！！من فضلك لا تستخدم اللغة العربية'));
		}
	}
	return $comment;
}
...
function bymt_ajax_comment(){
	if( isset( $_POST['action'] ) && $_POST['action'] == 'bymt_ajax_comment' && 'POST' == $_SERVER['REQUEST_METHOD']){
		...
		if ( !comments_open($comment_post_ID) ) {
			...
			err(__('受密码保护请先输入密码')); // 將 exit 改為錯誤提示
		} else {
		...
		if ( $user->exists() ) {
			...
			if ( get_option('comment_registration') || 'private' == $status )
				err(__('您必须先登陆才可以发表评论')); // 將 wp_die 改為錯誤提示
		}
		$comment_type = '';
		if ( get_option('require_name_email') && !$user->exists() ) {
			...
			elseif ( !is_email($comment_author_email))
				err(__('请填写一个有效的邮箱')); // 將 wp_die 改為錯誤提示
		}
		if ( '' == $comment_content )
...
function bymt_ajax_pagenavi(){ // pagenavi
	if( isset( $_GET['action'] ) && $_GET['action']== 'bymt_ajax_pagenavi' ){
		...
		if(!$postid || !$pageid){
			err(__('Error post id or comment page id.'));
		}
		// get comments
		$comments = get_comments('post_id='.$postid);	
		$post = get_post($postid);
		if(!$comments){
			err(__('Error! can\'t find the comments'));
		}
